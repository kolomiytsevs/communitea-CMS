...
from cmsdash.users.forms import (RegistrationForm, LoginForm, UpdateAccountForm,
                                 RequestResetForm, ResetPasswordForm, InviteUserForm)
from cmsdash import db, bcrypt
from cmsdash.models import User, Post
from flask_login import login_user, current_user, logout_user, login_required
from cmsdash.users.utils import send_reset_email, save_picture, send_invite_email
...
def account():
    ...
    return render_template('account.html', title='Account', image_file=image_file, updateform=updateform, inviteform=inviteform)


@users.route('/invite_user', methods=['GET', 'POST'])
@login_required
def invite():
    updateform = UpdateAccountForm()
    inviteform = InviteUserForm()
    if inviteform.validate_on_submit():
        send_invite_email(inviteform.invite_email.data, current_user)
        flash('Invite sent! Please inform the recipient.', 'info')
        return redirect(url_for('users.account'))
    image_file = url_for('static', filename='profilepics/' + current_user.image_file)
    return render_template('account.html', title='Account', image_file=image_file, updateform=updateform, inviteform=inviteform)
